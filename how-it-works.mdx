---
title: "How KiroGraph Indexes and Queries Your Codebase"
sidebarTitle: "How It Works"
description: "KiroGraph uses tree-sitter AST parsing to build a SQLite knowledge graph from your source code, then exposes it via MCP tools for instant symbol lookups and graph traversal."
---
...
Indexing is divided into five layers. The structural layer is always active. Every other layer is opt-in and is enabled through a flag in `.kirograph/config.json`.
...
| `interface` | TypeScript / Java / C# interfaces |
| `type_alias` | TypeScript `type` declarations |
| `enum` | Enumeration declarations |
| `variable` | Module-level variable and constant declarations |
...
| `decorator` | Class and method decorators |
...
| `lambda` | Lambda expressions (Python, Java, etc.) |
...
`calls` · `imports` · `exports` · `extends` · `implements` · `contains` · `references` · `instantiates` · `overrides` · `decorates` · `type_of` · `returns`
...
Every symbol stored in KiroGraph has the following fields:

| Field | Description |
|---|---|
| `id` | Stable internal integer ID |
| `name` | Short symbol name (e.g. `authenticate`) |
| `qualified_name` | Fully-qualified name including module path |
| `file` | Repo-relative file path |
| `line` / `end_line` | Source location |
| `kind` | One of the 26 node kinds listed above |
| `signature` | Function signature or type declaration text |
| `docstring` | Extracted doc comment (when `extractDocstrings: true`) |
| `embedding` | Float32 vector (when `enableEmbeddings: true`) |

Edges are stored with `source_id`, `target_id`, `kind`, `file`, and `line`. When `trackCallSites: true` is enabled, each `calls` edge also stores the exact call-site line numbers as a JSON array.
