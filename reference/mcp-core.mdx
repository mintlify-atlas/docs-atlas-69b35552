---
title: "Core and Navigation MCP Tools Reference"
sidebarTitle: "Core Tools"
description: "Reference for KiroGraph's always-on core tools (kirograph_context, kirograph_exec, kirograph_gain) and navigation tools for symbol search, graph traversal, and file listing."
---

KiroGraph's core tools are registered unconditionally — they appear in every `tools/list` response regardless of which feature flags are enabled. They cover the most frequent agent workflows: building task context, searching for symbols, tracing call chains, inspecting symbol details, and understanding impact before making changes. Navigation tools (`kirograph_search`, `kirograph_callers`, `kirograph_callees`) require `enableNavigation: true` and `trackCallSites: true` respectively, while status and file tools are bundled with the navigation flag.
...
<Note>
  `kirograph_exec` requires `enableShellExec: true` in your config. When the flag is `false`, the tool is not registered and `kirograph_compress` can be used instead for manual compression of command output.
</Note>
...
<Note>
  `kirograph_gain` requires `enableAgentUtils: true`. It reads from the same `TokenTracker` store that `kirograph_exec` writes to.
</Note>
...
## Status and File Tools (require `enableNavigation: true`)
