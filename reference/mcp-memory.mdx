---
title: "Memory MCP Tools Reference"
sidebarTitle: "Memory Tools"
description: "Reference for KiroGraph's 16 memory tools: store decisions, search observations, manage conflicts, review stale facts, and trigger Watchmen workspace synthesis."
---
...
**Watchmen response** (when `enableWatchmen: true` and the synthesis threshold is met):
...
| `targetFiles` | string[] | Project-relative paths to write the workspace brief to |
| `skillTargetDir` | string? | Present only when Kiro is detected (`.kiro/` exists). Path for individual `watchmen-<slug>.md` skill files. |

On receiving `watchmenReady: true`, the agent should: (1) call `kirograph_mem_search` for each kind, (2) write the workspace brief to each file in `targetFiles`, (3) if `skillTargetDir` is present write separate skill files for recurring procedures, otherwise embed a `## Recurring Procedures` section in the brief, (4) store a `kind: 'summary'` observation to reset the counter.
